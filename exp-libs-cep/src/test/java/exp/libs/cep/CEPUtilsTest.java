...
import exp.libs.cep.fun.impl.str.Cut;
import exp.libs.cep.fun.impl.str.Trim;
...
class CEPUtilsTest {
    ...
    public void testEval() throws Exception {
        ...
        Assertions.assertEquals(rst2, rst1);

        //非法运算调用测试
        Object rst3 = CEPUtils.eval("4 / 0");
        System.out.println("rst3: " + rst3);
        Assertions.assertEquals("Infinity", rst3.toString());

        //解析失败调用测试
        Object rst4 = CEPUtils.eval("x + y");	//没有声明的变量
        Assertions.assertEquals(CEPUtils.ERROR_RESULT, rst4);
    }
    ...
    public void testDeclare() throws Exception {
        ...
        Assertions.assertEquals(5.0, rst);
    ...
    public void testDiffCall() throws Exception {
        ...
        CEPUtils.register("TimeToSecond", "exp.libs.cep.fun.impl.time.Date2Sec");
        ...
        Assertions.assertEquals(nowDate.getTime(), sec);
        ...
        Assertions.assertEquals(CEPUtils.ERROR_RESULT, sec);
    ...
    public void testDiffStrCall() throws Exception {
        ...
        Assertions.assertEquals("abcd fg", str);
        ...
        str = CEPUtils.call("trim(  abcd fg   )");
        Assertions.assertEquals(CEPUtils.ERROR_RESULT, str);
    ...
    public void testIndefiniteParamNumCall() throws Exception {
        ...
        Assertions.assertEquals(19, now.toString().length());

        //当参数个数是1个时，该参数用于指定日期格式
        now = CEPUtils.call("now(\"yyyy-MM-dd\")");
        System.out.println(now);
        Assertions.assertEquals(10, now.toString().length());
    }
    ...
    public void testNestCall() throws Exception {
        ...
        Assertions.assertEquals("aaaa mm", str);
    }
