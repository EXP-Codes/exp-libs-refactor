...
final class _DBUtils {
	...
	private final static String TEMPLATE_DB_BEAN = "/exp/libs/db/sql/db-bean.tpl";
	
	private final static String TEMPLATE_PROXOOL = "/exp/libs/db/sql/proxool.tpl";
	...
	private static String createProxoolXml(DataSourceBean ds) throws Exception {
		FileTemplate tpl = new FileTemplate(TEMPLATE_PROXOOL, Charset.ISO);
	...
	protected static void createBeanFromPDM(String pdmPath, String packageName,
		...
		List<Node> elms = x.selectNodes(document);

		//析取所有表对象
		for (Iterator<Node> elmIts = elms.iterator(); elmIts.hasNext();) {
			Element eTable = (Element) elmIts.next();
	...
	private static String createBeanData(String tableName, 
		...
		//取类模板
		FileTemplate beanClazz = new FileTemplate(TEMPLATE_DB_BEAN, Charset.ISO);
